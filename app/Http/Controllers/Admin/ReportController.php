...
class ReportController extends Controller
{
    ...
    public function revolving_fund_reports(Request $request){
        ...
        $allocations = RevolvingFundAllocation::get();
        if ($request->centre_id) {
            $allocations = $allocations->where('centre_id', $request->centre_id);
        }
        if ($request->season_id) {
            $allocations = $allocations->where('season_id', $request->season_id);
        }
   
        return view('admin.reports.revolving-fund-reports', compact('centres','seasons','allocations'));
    }
